...
| --- | --- | --- |
| The video | `ffprobe` and `ffmpeg`, as argument lists, no shell | Absolute paths so a name cannot read as an option. `-format_whitelist` names the media formats a script will open (MP4, MOV, Matroska, WebM, AVI, MPEG-TS, MXF, and the common audio formats), so a playlist, an index, or a script named like a video is refused before a demuxer reads a byte; `-protocol_whitelist file` keeps anything inside the media off the network; `probe()` refuses playlist, index, and script format names as a second layer, and every script probes before it cuts. `audio_energy.py` refuses media over 6 hours unless told otherwise and caps the extract to that length. `frames.py` counts before it allocates, refuses more than 80 frames, and generates times by index, so a huge time cannot stall an accumulating loop. `cut_previews.py` refuses more than 20 items or a clip over 60 seconds without `--force`. `thumbnail_mockup.py` crops before it scales, so a 4096x2 frame cannot become a gigapixel image, and refuses frames that are not video-shaped. Every `ffmpeg` call has a timeout. |
| Output files | every script | A bundle can carry a file named like an output (`transcript_compact.txt`, `thumb_mock.png`, a `frames/` or `previews/` folder) as a hard link, a symlink, or a junction pointing at the source recording. Every output name is refused when it is any kind of link or has more than one hard link, every output is written to a fresh temp file beside it and moved into place with a replace, and the `frames/` and `previews/` folders must be real folders inside the episode folder. Nothing is ever written through an existing name. |
...
| The plan | `load_plan` in `_common.py`, then `check_plan.py` and `cut_previews.py` | Files over 5 MB and plans with more than 200 clips are refused. The plan must be an object with an object `episode` and a list of object `clips`. Every time value must be finite, non-negative, and under hour 100; `nan`, `inf`, and negatives fail validation and never reach `ffmpeg`. `publish_order` must be a whole number from 1 to 999 in both scripts because it names a file. A wrong type anywhere, and JSON nested too deep to parse, is a `FAIL` line, never a traceback. The overlap check reports one line per clip. |
