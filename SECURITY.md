...
| Input files by type | `parse_captions`, `load_plan`, `thumbnail_mockup.py` | A caption or plan file must be a regular file, and the read itself is capped, so a symlink to a device or a pipe cannot pass the size check by reporting zero bytes. A frame passed with `--frame` is opened as JPEG or PNG only, so a PostScript file named `.jpg` never reaches Ghostscript through Pillow. |
...
| The plan | `load_plan` in `_common.py`, then `check_plan.py` and `cut_previews.py` | Files over 5 MB and plans with more than 200 clips are refused. The plan must be an object with an object `episode` and a list of object `clips`. Every time value must be finite, non-negative, and under hour 100; `nan`, `inf`, and negatives fail validation and never reach `ffmpeg`. `publish_order` must be a whole number from 1 to 999 in both scripts because it names a file. Text fields must be text and list fields lists of text; a wrong type anywhere, and JSON nested too deep to parse, is a `FAIL` line, never a traceback. The overlap check reports one line per clip. `--force` on the preview cutter is never used unless the creator asks (SKILL.md rule 4). |
