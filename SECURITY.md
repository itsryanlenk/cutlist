...
| Console output | every script | Every path a script prints or names in a refusal, and every line a program prints, is cleaned of control, invisible, and line-break characters, so a folder name cannot forge a status line in the output the agent reads. Flagged lines in `markers.txt` are named by time stamp only; their text is never echoed, because a markers file could be a link to any file on the machine. |
...
- `install.sh` or `install.ps1` deleting anything but the one skill folder they own, or
  `rebrand.py` writing outside the repository or through a link, its own `brand.json` included.
